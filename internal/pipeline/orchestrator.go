...
func (o *Orchestrator) Execute(ctx context.Context, cfg Config) (*Result, error) {
	...
	if !cfg.DrumsOnly && stemResult.PianoPath != "" {
		...
		// LLM-FIRST ARCHITECTURE: All Strudel code is generated by Python AI
		// Go only does: analysis → call Python AI → return code
		o.progress.StageComplete("Generating Strudel code via AI...")

		// Prepare arguments for AI code generator
		aiArgs := []string{
		...
		// Call Python AI code generator (LLM generates ALL code)
		aiResult, aiErr := o.runner.RunScript(ctx, "ai_code_generator.py", aiArgs...)
		if aiErr != nil {
...
processDrums:
	...
	if cfg.DrumsOnly && drumResult != nil {
		...
		drumResult, drumErr := o.runner.RunScript(ctx, "ai_code_generator.py", drumArgs...)
		if drumErr != nil {
...
`, result.BPM, result.BPM)
			...
			result.StrudelCode = drumResult.Stdout
		}
	} else if drumResult != nil && strudelCode != "" && !strings.Contains(strudelCode, "drums") {
		// If AI didn't include drums, append them
		// Call AI to generate drum section separately
		drumArgs := []string{
			stemResult.DrumsPath,
			"--drums-only",
			"--bpm", fmt.Sprintf("%.0f", result.BPM),
		...
		drumCodeResult, _ := o.runner.RunScript(ctx, "ai_code_generator.py", drumArgs...)
		if drumCodeResult != nil && drumCodeResult.Stdout != "" {
			// Combine melodic + drums
			result.StrudelCode = strudelCode + "\n// Drums\n" + drumCodeResult.Stdout
		} else {
			result.StrudelCode = strudelCode
		}
	} else {
