...
func (o *Orchestrator) Execute(ctx context.Context, cfg Config) (*Result, error) {
	...
	if !cfg.DrumsOnly && stemResult.PianoPath != "" {
		...
		// LLM-FIRST ARCHITECTURE: Ollama generates ALL Strudel code
		// Go does: analysis → section detection → call Ollama codegen → return code

		// Run smart analysis to detect sections (energy, structure)
		smartAnalysisPath := filepath.Join(ws.Dir, "smart_analysis.json")
		_, smartErr := o.runner.RunScript(ctx, "smart_analyze.py", stemResult.PianoPath, smartAnalysisPath)
		if smartErr != nil {
		...
		// Prepare arguments for Ollama code generator
		aiArgs := []string{
		...
		// Pass sections data if available
		if fileExists(smartAnalysisPath) {
			aiArgs = append(aiArgs, "--sections-json", smartAnalysisPath)
		}

		// Add cache key for ClickHouse learning
		if cacheKey != "" {
			aiArgs = append(aiArgs, "--track-hash", cacheKey)
		}

		// Add genre if detected
		if result.Genre != "" {
			aiArgs = append(aiArgs, "--genre", result.Genre)
		}

		// Add drum kit
		if cfg.DrumKit != "" {
			aiArgs = append(aiArgs, "--drum-kit", cfg.DrumKit)
		}

		// Add notes JSON path if available
		if fileExists(ws.NotesJSON()) {
...
// BPM: %.0f, Key: %s
// AI generation failed, minimal placeholder
...
`, analysisResult.BPM, analysisResult.Key, analysisResult.BPM)
			...
			// Parse JSON output from AI generator
			var aiOutput struct {
...
processDrums:
	...
	// Final output: AI already generated complete code including drums
	// No Go-based generation - all code comes from Python AI
	kit := cfg.DrumKit
	if kit == "" {
		kit = "tr808"
	}
	...
	if cfg.DrumsOnly && drumResult != nil {
		// Drums-only: call AI to generate drum pattern
		drumArgs := []string{
			stemResult.DrumsPath,
			"--drums-only",
			"--bpm", fmt.Sprintf("%.0f", result.BPM),
			"--drum-kit", kit,
		}
		if fileExists(ws.DrumsJSON()) {
			drumArgs = append(drumArgs, "--drums-json", ws.DrumsJSON())
		}
		drumResult, drumErr := o.runner.RunScript(ctx, "ollama_codegen.py", drumArgs...)
		if drumErr != nil {
			// Fallback minimal drums
			result.StrudelCode = fmt.Sprintf(`// MIDI-grep output (drums only)
// BPM: %.0f

setcps(%.0f/60/4)

$: s("bd sd hh hh").bank("RolandTR808")
`, result.BPM, result.BPM)
		} else {
			result.StrudelCode = extractStrudelCode(drumResult.Stdout)
		}
	} else if drumResult != nil && strudelCode != "" &&
		!strings.Contains(strings.ToLower(strudelCode), "drums") &&
		!strings.Contains(strudelCode, `.bank(`) &&
		!strings.Contains(strudelCode, `s("bd`) {
		// If AI didn't include drums, append a simple drum pattern
		o.progress.Warning("AI didn't generate drums — appending default drum pattern")
		drumPattern := fmt.Sprintf("\n// Drums\n$: s(\"bd sd hh hh bd sd hh oh\")\n  .bank(\"%s\")\n  .gain(0.70)", kit)
		result.StrudelCode = strudelCode + drumPattern
	} else {
		// AI generated complete code (melodic + drums)
		result.StrudelCode = strudelCode
	}
...
// extractStrudelCode parses AI code generator output, extracting the "code" field
// from JSON output or returning the raw string if it's not JSON.
func extractStrudelCode(stdout string) string {
	var parsed struct {
		Code string `json:"code"`
	}
	if json.Unmarshal([]byte(stdout), &parsed) == nil && parsed.Code != "" {
		return parsed.Code
	}

	// Try to find JSON object in the output (may have non-JSON prefix from imports/logging)
	if idx := strings.Index(stdout, `{"code"`); idx >= 0 {
		jsonStr := stdout[idx:]
		// Find the end of the JSON object
		if endIdx := strings.LastIndex(jsonStr, "}"); endIdx >= 0 {
			jsonStr = jsonStr[:endIdx+1]
			if json.Unmarshal([]byte(jsonStr), &parsed) == nil && parsed.Code != "" {
				return parsed.Code
			}
		}
	}
...
func detectStyle(analysisResult *analysis.Result, cleanup *midi.CleanupResult) string {
	candidates := detectStyleCandidates(analysisResult, cleanup)
	if len(candidates) > 0 {
...
func detectStyleCandidates(analysisResult *analysis.Result, cleanup *midi.CleanupResult) []StyleCandidate {
	bpm := analysisResult.BPM
	key := analysisResult.Key
	...
	// BPM-based scoring
	addScore := func(s string, score float64) {
	...
	if bpm >= 125 {
		addScore("house", 0.9)
		addScore("trance", 0.8)
		addScore("electronic", 0.7)
		if noteDensity > 4 {
			// Boost trance for very dense
			for i := range scores {
				if scores[i].style == "trance" {
					scores[i].score += 0.2
				}
			}
		}
		...
		// Medium-fast
		addScore("electronic", 0.8)
		addScore("funk", 0.75)
		addScore("synthwave", 0.7)
		addScore("house", 0.6)
	} else if bpm >= 90 {
		// Medium
		addScore("synth", 0.8)
		addScore("electronic", 0.7)
		addScore("darkwave", 0.65)
		addScore("funk", 0.6)
	} else if bpm >= 70 {
		// Medium-slow
		addScore("jazz", 0.8)
		addScore("lofi", 0.75)
		addScore("soul", 0.7)
		addScore("cinematic", 0.65)
	} else if bpm >= 50 {
		// Slow
		addScore("ambient", 0.8)
		addScore("cinematic", 0.75)
		addScore("newage", 0.7)
		addScore("lofi", 0.6)
	} else {
		// Very slow
